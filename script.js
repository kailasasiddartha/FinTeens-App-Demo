...
function setupOnboarding() {
  ...
  const btnSkip = $("btnSkipOnboard");
  if (!btnStart || !btnSkip) return;
...
function setupMentor() {
  const askBtn = $("btnAskMentor");
  const input = $("mentorInput");
  const clearBtn = $("btnClearMentor");
  if (!askBtn || !input || !clearBtn) return;

  askBtn.addEventListener("click", sendMentorQuestion);
...
function setupWallet() {
  ...
  const withBtn = $("btnWithdraw");
  if (!depBtn || !withBtn) return;
...
function setupTrading() {
  ...
  // initial chart
  if (select.value) {
    updateMarketChart(select.value);
  }
...
function handleTrade(type) {
  ...
  if (type === "buy") {
    ...
    if (!holding) {
      holding = {
        id: assetId,
        name: marketAssets.find(a => a.id === assetId).name,
        qty: 0,
        avgPrice: 0
      };
...
function updateAllUI() {
  ...
  $("xpFill") && ($("xpFill").style.width = `${Math.min(100, xpMod)}%`);
  ...
  $("quizScore") && ($("quizScore").textContent = quizScore);
